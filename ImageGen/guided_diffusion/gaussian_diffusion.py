...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            with torch.no_grad(): 
                out = self.p_sample(model=model, x=img, t=time, measurement = measurement, alpha=alpha,)
    ...
    def p_sample(self, model, x, t, measurement=None,alpha=5,):
        raise NotImplementedError

    def p_mean_variance(self, model, x, t,measurement=None,alpha=5,):
        ...
        y = measurement
        if y.shape[-1] != x.shape[-1]: # for super resolution
            y = F.interpolate(y, size=x.shape[-2:], mode='bicubic', align_corners=False)
        noise_h_term =  (x - _extract_into_tensor(self.sqrt_alphas_cumprod, t, x.shape)* y) / (_extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x.shape)) - model_output
        model_output = model_output + noise_h_term * _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x.shape)**alpha
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t,measurement=None,alpha=5):
        out = self.p_mean_variance(model, x, t, measurement=measurement,alpha=alpha,)
        sample = out['mean']

        noise = torch.randn_like(x)
        # if t != 0:  # no noise when t == 0
        # consider t is a tensor
        if (t != 0).all():
...
class DDIM(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=0.0, measurement=None):
