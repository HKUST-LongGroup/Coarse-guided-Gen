...
class CogVideoXImageToVideoCGGPipeline(CogVideoXImageToVideoPipeline):
    r"""
    Pipeline for image-to-video generation using CogVideoX combined with Time to Move (CGG).
    ...
    def __call__(
        ...
        if motion_signal_mask_path is None:
            raise ValueError("`motion_signal_mask_path` is required for CGG.")
        if motion_signal_video_path is None:
            raise ValueError("`motion_signal_video_path` is required for CGG.")
        ...
        x_0 = ref_latents.to(dtype=torch.bfloat16)
        with self.progress_bar(total=len(timesteps)) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                w_1 = math.pow(beta_prod_t,w1)
                w_2 = math.pow(beta_prod_t,w2)
                v_pred = noise_pred
                v_ref = (latents * alpha_prod_t - x_0) / beta_prod_t
                v_final = (
                    (v_pred + w_1 * (v_ref - v_pred)) * motion_mask
                        +
                    (v_pred + w_2 * (v_ref - v_pred)) * background_mask
                )
                latents, old_pred_original_sample = self.scheduler.step(v_final, t, latents, **extra_step_kwargs, return_dict=False)
