...
class WanImageToVideoCGGPipeline(WanImageToVideoPipeline):
    r"""
    Pipeline for image-to-video generation using Wan with Time-To-Move (CGG) conditioning.
    ...
    def __call__(
        ...
        Args:
            ...
            motion_signal_mask_path (`str`):
                Path to the mask video file containing the motion mask of CGG.
        ...
        if motion_signal_video_path is None:
            raise ValueError("`motion_signal_video_path` must be provided for CGG.")
        if motion_signal_mask_path is None:
            raise ValueError("`motion_signal_mask_path` must be provided for CGG.")
        ...
        with self.progress_bar(total=len(timesteps)) as progress_bar:
            for i, t in enumerate(timesteps):
                t_idx = self.scheduler.timesteps.tolist().index(t.item())
                sigma_t = self.scheduler.sigmas[t_idx]
                sigma_next = self.scheduler.sigmas[t_idx + 1]
                ...
                if self.do_classifier_free_guidance:
                    with current_model.cache_context("uncond"):
                        ...
                        v_pred = noise_uncond + current_guidance_scale * (noise_pred - noise_uncond)
                
                w_1 = math.pow(sigma_t,w1)
                w_2 = math.pow(sigma_t,w2)
                v_ref = (latents - x_0) / sigma_t
                latents = latents - dt * (
                    (v_pred + w_1 * (v_ref - v_pred)) * motion_mask
                                        +
                    (v_pred + w_2 * (v_ref - v_pred)) * background_mask
                )
